...
from appapi.services.backtest.runner import get_quant_runtime_runner


def run_backtest(request: BacktestRunRequest) -> BacktestRunResponse:
    """业务功能: 同步调用 runner 执行一次回测。"""
    payload = get_quant_runtime_runner().run(build_runner_payload(request))
...
def submit_backtest_job(request: BacktestRunRequest) -> BacktestJobSubmitResponse:
    ...
    payload = get_quant_runtime_runner().submit(build_runner_payload(request))
...
def get_backtest_job_status(job_id: str) -> BacktestJobStatusResponse:
    ...
    payload = get_quant_runtime_runner().status(job_id)
...
def get_backtest_job_result(job_id: str) -> BacktestRunResponse:
    ...
    payload = get_quant_runtime_runner().result(job_id)

...
def test_load_environment_config_resolves_common_env_items(tmp_path):
    environ = {
        ...
        "AUTH_DATABASE_DSN": "postgresql://test:test@db:5432/ctp_research",
    ...
    assert config.auth_database_dsn == "postgresql://test:test@db:5432/ctp_research"
    assert config.auth_token_secret == "test-secret"
...
def test_default_environment_config_points_at_repo_root():
    ...
    assert config.auth_database_dsn == (
        "postgresql://postgres:postgres@localhost:5432/ctp_research"
    )
    assert config.auth_token_secret == "local-development-auth-secret"
